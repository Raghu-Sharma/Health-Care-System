...
def helper(dis):
    ...
    def parse_list(df, disease, column, default):
        val = df.loc[df['Disease'] == disease, column]
        if not val.empty and isinstance(val.iloc[0], str):
            return literal_eval(val.iloc[0])
        return [default]

    med = parse_list(medications, dis, 'Medication', 'No medication available')
    die = parse_list(diets, dis, 'Diet', 'No diet available')
    # ============== yaha tak
...
symptoms_dict = {'itching': 0, 'skin rash': 1, 'nodal skin eruptions': 2, 'continuous sneezing': 3, 'shivering': 4,
                 ...
                 'irregular sugar level': 23, 'cough': 24, 'high fever': 25, 'sunken eyes': 26, 'breathlessness': 27,
                 'sweating': 28, 'dehydration': 29, 'indigestion': 30, 'headache': 31, 'yellowish skin': 32,
                 ...
                 'enlarged thyroid': 71, 'brittle nails': 72, 'swollen extremeties': 73, 'excessive hunger': 74,
                 'extra marital contacts': 75, 'drying and tingling lips': 76, 'slurred speech': 77, 'knee pain': 78,
                 ...
                 'fluid overload.1': 117, 'blood in sputum': 118, 'prominent veins on calf': 119, 'palpitations': 120,
                 'painful walking': 121, 'pus filled pimples': 122, 'blackheads': 123, 'scurring': 124,
                 'skin peeling': 125, 'silver like dusting': 126, 'small dents in nails': 127,
...
def predict():
    if request.method == 'POST':
        ...
        return render_template('index.html', predicted_disease=predicted_disease, dis_des=desc, dis_pre=pre,
                               dis_meds=med,
                               dis_diet=die, dis_workout=wrkout)
...
def feedback():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('feedback.db')
        cursor = conn.cursor()
        cursor.execute("INSERT INTO feedbacks (name, feedback, rating) VALUES (?, ?, ?)",
    ...
    conn = sqlite3.connect('feedback.db')
    cursor = conn.cursor()
    cursor.execute("SELECT name, rating, feedback FROM feedbacks ORDER BY id DESC")
